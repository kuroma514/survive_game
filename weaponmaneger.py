...
class WeaponManager:
    ...
    def add_weapon(self, weapon_type, cooltime, AT, bullet_speed, size):
        """武器を登録し、クールタイムを設定"""
                    ...
                    if weapon_type == "Waterball":
                        new_weapon = Waterball(
                            self.weapon_group, "Water Effect and Bullet", cooldown_info[
                                "AT"], cooldown_info["bullet_speed"], cooldown_info["size"], 3000, self.player,  # bullet_speed = 2
                            self.player.rect.centerx, self.player.rect.centery)
                    elif weapon_type == "Syuriken":
                        new_weapon = Syuriken(
                            self.weapon_group, "syuriken", cooldown_info[
                                "AT"], cooldown_info["bullet_speed"], cooldown_info["size"], 2000, self.player,  # bullet_speed = 3.5
                            self.player.rect.centerx, self.player.rect.centery)
                    elif weapon_type == "Bom":
                        new_weapon = Bom(
                            self.weapon_group, "bom", cooldown_info[
                                "AT"], cooldown_info["bullet_speed"], cooldown_info["size"], 3500, self.player,  # bullet_speed = 3.5
                            self.player.rect.centerx, self.player.rect.centery)
                    elif weapon_type == "Beam":
                        new_weapon = Beam(
                            self.weapon_group, "beam", cooldown_info[
                                "AT"], cooldown_info["bullet_speed"], cooldown_info["size"], 4000, self.player,  # bullet_speed = 3.5
                            self.player.rect.centerx, self.player.rect.centery)
